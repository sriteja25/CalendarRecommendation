...
class GPTService {
    ...
    func getResponse(event: EKEvent, startWeather: HourlyData, endWeather: HourlyData, locationName: String , completionHandler: @escaping (String?, Error?) -> Void) {
        ...
        let prompt = "I am attending an event named \(event.title) at \(locationName). It starts at \(event.startDate) and ends at \(event.endDate). The weather is likely to be \(startWeather.temp) degrees centigrade at the start of event with \(startWeather.weather[0].description) and \(endWeather.temp) degrees centigrade at the end of event with \(endWeather.weather[0].description). First recommendation: Give me a very very specific clothing recommendation (Shirt, pant, short, frock, saree, cotton, wool, nylon etc) for weather Second Receommndation: if an umbrella or jacket are necessary in 5 words. Third Recommendation: Anything based on event title \(event.title) a small random fact about the event city. Give the output properly formatted without the word recommendation and all the points numbered and ready to display in a UILabel."
        print(prompt)
